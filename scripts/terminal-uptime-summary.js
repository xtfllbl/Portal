(function () {
  ...
  function render() {
    ...
    host.innerHTML = `<header class="up-summary-heading"><div class="up-summary-title"><h2>Service Uptime</h2></div><div class="up-actions"><a class="up-link-button" href="${V.esc(historyUrl())}">View history${V.icon('arrow_outward')}</a></div></header><div class="up-summary-grid">${days.length ? days.map(r => V.cell(r, sn, true)).join('') : '<p class="up-summary-empty">No terminal history in the last 7 days.</p>'}</div><footer class="up-summary-footer"><div class="up-legend" aria-label="Uptime legend">${V.legend()}</div><span class="up-meta" title="${new Date(data.observedAt).toISOString()}">${V.esc(zone.replace(/_/g, ' '))} · Updated ${new Date(data.observedAt).toLocaleString('en-GB', { timeZone: zone, month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</span></footer><p class="up-error" role="alert" tabindex="-1" hidden></p>`;
  }
  function load() {
    try {

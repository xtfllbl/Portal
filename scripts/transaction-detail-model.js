...
'use strict';
const resultLabel=row=>({completed:'Completed',failed:'Failed',pending:'Pending'})[row.status]||'Unknown';
function describe(row){
 const payload={...(row.detail||{})};
 const fields={paywizardId:'paywizardId',mid:'mid',transType:'type',amount:'amount',tranTime:'processorTime',terminalDateTime:'terminalTime',terminalTimezone:'timezone',cardNo:'cardPanMasked',sn:'sn',transId:'transId',invoiceNum:'invoiceNumber',traceNum:'trace',approvalCode:'approvalCode',tId:'tid',tci:'tci',merchantName:'merchantName',merchantAddress:'terminalName',batchNum:'batch',receiptEmail:'email',oriTransId:'originalTransId',externalOrderNo:'externalOrderNo',rrn:'rrn'};
 ...
 if(payload.transType==='Failed')payload.transType='Unknown';
    const groups=[
      {title:"Transaction",fields:[["Transaction Type",payload.transType],["Transaction Status",resultLabel(row)],["Transaction Amount",`${row.currency} ${row.amount}`],["Currency Code",payload.currencyCode],["Transaction Time",payload.tranTime],["Terminal Time",payload.terminalDateTime],["Terminal Timezone",payload.terminalTimezone],["Receipt Email",payload.receiptEmail],["APPR Code",payload.approvalCode,"mono"],["Reference No.",payload.rrn,"mono"],["Transaction ID",payload.transId,"mono"],["Invoice No.",payload.invoiceNum,"mono"],["Trace No.",payload.traceNum,"mono"],["Batch No.",payload.batchNum,"mono"],["Checkout ID",payload.checkoutId,"mono"],["PAYWizard ID",payload.paywizardId,"mono"]]},
      ...
      {title:"Original",fields:[["Original TransIndexCode",payload.oriTransIndexCode,"mono"],["Original Invoice No.",payload.oriInvoiceNum,"mono"],["Original Transaction ID",payload.oriTransId,"mono"],["Original Reference",payload.oriRrn,"mono"]]},
 ...
 groups[0].fields.push(['Trans Log ID',row.transLogId],['Trans Index Code',row.transIndexCode]);
 ...
 const summary=[['Transaction Time',payload.tranTime],['External Order No.',payload.externalOrderNo],['Merchant ID',payload.merchantId??row.mid],['Terminal ID',payload.tId],['APPR Code',payload.approvalCode],['Reference Number',payload.rrn],['Transaction ID',row.paywizardId],['Terminal Name',row.terminalName]];

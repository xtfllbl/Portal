...
- 参考 Spark 对话 19977 的 Receipt 信息结构；保留金额、付款日期、费用描述、备注、卡种尾号和 Invoice。视觉以 Spark 中邮件 19977 的实际呈现为准：WizarPOS 左上标识、右上 RECEIPT、居中金额、Description/Amount 双列、蓝色期数、浅黄虚线备注及左右排列的付款方式与 Invoice。标识复用本地既有邮件资料 `../邮件HTML/HTML文件/wizarpos-logo.png` 的黑蓝色 WizarPOS 图片，与实际收据外观一致。预览使用同目录 `billingReceiptLogo.png`；正式发送必须通过 `logoUrl` 指定托管 HTTPS 地址或 CID 内嵌附件地址。

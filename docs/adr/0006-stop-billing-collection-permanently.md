...
Operations needs to stop further collection against an issued Billing Record. The agreed action permanently stops subsequent collection, including payment through its existing link, instead of temporarily pausing automatic charges; it preserves successful payments, unpaid amounts and the stop reason, does not mark the unpaid amounts as Paid, and does not automatically refund previous payments. This extends ADR 0003's earlier exclusion of subscription management with a narrowly scoped permanent collection stop, available to WizarPOS Provider alongside issued-bill management in Billing Overview.

Stopping collection immediately prevents new payment attempts. An attempt already submitted is allowed to settle, and its outcome remains recorded without automatic refund; a late success must not reopen collection. Confirmation shows the paid and remaining amounts and requires a stop reason.

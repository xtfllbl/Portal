...
Chinese product and engineering handoff: [无人值守终端广告管理需求文档](advertising-requirements.md).
...
The idle-screen frame follows the supplied `OPC/0.OPC_standby.html` template: black background, Inter typography, SN and live local time, seven original payment-brand SVGs arranged 4 + 3, the original white/blue wizarPOS image, and merchant/TID footer. Advertising replaces only the central Ready for Payment content; the empty playlist restores that text. Q3V uses the template's width-based proportions at its native 480 × 800 resolution, assigning the additional height to the central area. As confirmed on 2026-09-18, Q3min adapts spacing and element sizes for 480 × 480 while preserving order and artwork proportions; this is an approved preview adaptation, not a separately supplied real-device square-screen template. Fit/Fill applies only within the central advertising area.
...
Publication validates selected IDs and blocks overlapping terminal assignments or stores already claimed by another non-stopped campaign. Empty stores can publish and enroll terminals later. Published scopes, rather than draft edits, determine membership. Occupied newcomers retain their original campaign while that campaign's published scope still includes them; the affected campaign shows Target conflict in Campaigns, and its editor lists the conflicting SNs and owners. Stop Campaign releases existing assignments and disables future enrollment; this action is available on every non-stopped published campaign, including empty-store campaigns. Stopping the conflicting campaign lets the pending store scope cover the released terminal.
